...
We want to resolve the Maxwell's equations presented in xref:../Maxwell.adoc[Maxwell modeling] and compare them with some exact solutions given by C. Trophime using Bmap.
...
The solver and preconditionner are the ones detail in xref:../Strategy.adoc[Variational formulations and resolution strategy].
