...
:page-cards-order: 1
:description: Modeling foundations of Maxwell equations for static magnetic field problems.
...
For solver strategies and detailed weak forms, continue with xref:Strategy.adoc[Variational formulations and resolution strategy].

For the full coupled electro-magnetic model with stem:[A], stem:[V], stem:[J], stem:[E] and temperature-dependent properties, see xref:resistive-magnets-coupling.adoc[Coupled stem:[A-V-J-E-T] model for resistive magnets].

For benchmark methodology and validation references, continue with xref:benchmarking.adoc[Benchmarking and validation].
