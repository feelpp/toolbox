...
:description: We simulate a pressure wave propagation into a canal with a fluid-structure interaction model.
...
link:../index.adoc[fluid-structure interaction model].
...
By using the link:../../CSM/index.adoc[1D reduced model], named generalized string and explained by <<Chabannes>>, we didn't need to define the
...
* We add also the link:../../CSM/index.adoc[specific coupling conditions], obtained from the axisymmetric reduced model, on stem:[\Gamma^*_{fsi}]
