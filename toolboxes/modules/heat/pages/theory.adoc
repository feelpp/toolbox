...
==== Newton non-linear iteration

The previous integral equation can be included in the variational formulation of the PDE as a boundary condition on the heat flux stem:[q(x)=\Big(-k \nabla T \cdot n \Big)(x)]:

====
[stem]
++++
q(x) = \epsilon(x) \Bigg( E_b(x) - \int_{\partial \Omega_R} E_b(x') dF_{x-x'} + \int_{\partial \Omega_R} \Big( \frac{1}{\epsilon(x')}-1\Big)q(x') dF_{x-x'}
	\Bigg).
++++
====

The linearization of the boundary condition generates the following terms to be added to the Jacobian matrix stem:[DF(T)], at iteration stem:[k],
...
The following term is added to the residual stem:[F(T)] at iteration stem:[k], where we denoted by stem:[A] and stem:[A'] the areas surrounding points stem:[x] and stem:[x'].

====
[stem]
++++
\begin{multline}
F(T_k) = \int_A \epsilon(x) \sigma T^4_k \varphi - \int_A \epsilon(x) \Big[\int_{A'} \sigma T^4_k(x') dF_{dA-dA'}\Big] \varphi(x) +\\
	+ \int_A \epsilon(x) \Big[\int_{A'} \Big( \frac{1}{\epsilon(x')} -1\Big)(-k\nabla T_k \cdot \vec{n})  dF_{dA-dA'} \Big]\varphi(x).
\end{multline}
++++
====
In practice, the surface of the cavity is divided into smaller surfaces and the view factors stem:[F_{ij}] are often already computed and stored for all pairs  stem:[ij] of (finite) subsurfaces. In that case, the differential stem:[dF_{dA-dA'}] can be substituted by the ratio stem:[F_{ji}/|A_i|]. The meaning of  stem:[dF_{dA-dA'}] is ``the ratio (diffuse energy leaving $dA$ and intercepted by stem:[dA'])/(total energy leaving stem:[dA]) '', and the ratio stem:[F_{ji}/|A_i|] corresponds to the ratio of (energy leaving surface $j$ and intercepted by surface stem:[i])/(total energy leaving surface stem:[j]) divided by the area of the intercepting surface. The idea behind the substitution is that integrals stem:[ \Big(\int_{A'} \sigma T^4_k(x') dF_{dA-dA'}\Big) ] and stem:[\Big(\int_{A'} \Big( \frac{1}{\epsilon(x')} -1\Big)(-k\nabla T_k \cdot \vec{n})  dF_{dA-dA'} \Big)] produce values for all stem:[x\in A], by attributing them a portion of incident radiation which is proportional to the view factor. By dividing stem:[F_{ji}] by stem:[|A_i|] we construct a density for the view factor, constant over the whole receiving surface, to average the incoming radiation. We hence make the approximation that stem:[ dF_{dA-dA'} = \cos(\theta_i)\cos(\theta_j)/(\pi S^2)dA' \approx F_{ji}/|A_i| dA' = F_{ij}/|A_j| dA' ].
