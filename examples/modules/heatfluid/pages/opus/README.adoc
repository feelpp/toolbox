...
As the periodicity helps to dissipate the heat in both directions of the PCB, the results
are attended to be higher than the true results.
...
<script type="text/javascript">
  Plotly.d3.csv("https://girder.math.unistra.fr/api/v1/file/5afea86fb0e957402704804a/download",
    function(err,rows) {
      var data = [{
        name: 'IC2 results of test 1',
        type: 'scatter',
        x: plotly_unpack(rows,'time'),
        y: plotly_unpack(rows,'test1-s1'),
        showlegend: true,
        line: { color: '#CC3333' }
      }{
        name: 'Output results of test 1',
        type: 'scatter',
        x: plotly_unpack(rows,'time'),
        y: plotly_unpack(rows,'test1-s2'),
        showlegend: true,
        line: { color: '#3333CC' }
      }];
...
[source,cfg]
----
include::{examplesdir}/opus/test1.cfg[]
----
