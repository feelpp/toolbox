...
we have stem:[v_{inlet} = D y(height-y)] 
To determine D, we know that for stem:[y=\frac{height}{2}] we have the maximal velocity, so 
stem:[D=\frac{v_{max}}{\frac{height}{2}\left(height-\frac{height}{2}\right)}]
