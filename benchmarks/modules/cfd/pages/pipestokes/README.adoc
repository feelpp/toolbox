...
we have stem:[v_{inlet} = D y(height-y)]. To determine D, we know that for stem:[y=\frac{height}{2}] we have the maximal velocity, so 
...
directory=tp   <1>
[fluid] <2>
filename=$cfgdir/pipestokes.json <3>
mesh.filename=$cfgdir/pipestokes.geo <4>
gmsh.hsize=0.1 <5>
...
----
mpirun -np 4 feelpp_toolbox_fluid_2d --config-file /usr/local/share/feelpp/testcases/CFD/pipestokes/pipestokes.cfg --fluid.gmsh.geo-variables-list="height=3" --json-editions Parameters.height:3
----
* For the pressure
...
We saw that in CFG file, we can choose between two decomposition LU and Gasm, in the the theory, the option Gasm is faster than LU, in fact Gasm decompose the domaine and it use LU in every part in parallel. 
